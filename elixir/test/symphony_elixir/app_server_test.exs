defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server executes supported dynamic tool calls and returns the tool result" do
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case \"$count\" in
          ...
          4)
            ...
            printf '%s\\n' '{\"id\":102,\"method\":\"item/tool/call\",\"params\":{\"name\":\"lark_task_api\",\"callId\":\"call-90a\",\"threadId\":\"thread-90a\",\"turnId\":\"turn-90a\",\"arguments\":{\"method\":\"GET\",\"path\":\"/open-apis/task/v2/tasks/task-90a\",\"query\":{\"include_resource\":false}}}}'
            ;;
          5)
            printf '%s\\n' '{\"method\":\"turn/completed\"}'
            exit 0
      ...
      tool_executor = fn tool, arguments ->
        ...
        %{
          "success" => true,
          "output" => ~s({"code":0,"data":{"task":{"guid":"task-90a"}}}),
          "contentItems" => [
            %{
              "type" => "inputText",
              "text" => ~s({"code":0,"data":{"task":{"guid":"task-90a"}}})
            }
          ]
        }
      end
      ...
      assert_received {:tool_called, "lark_task_api",
                       %{
                         "method" => "GET",
                         "path" => "/open-apis/task/v2/tasks/task-90a",
                         "query" => %{"include_resource" => false}
                       }}
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 payload["id"] == 102 and
                   ...
                   get_in(payload, ["result", "output"]) ==
                     ~s({"code":0,"data":{"task":{"guid":"task-90a"}}})
               else
                 false
               end
             end)
  ...
  test "app server emits tool_call_failed for supported tool failures" do
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case \"$count\" in
          ...
          4)
            ...
            printf '%s\\n' '{\"id\":103,\"method\":\"item/tool/call\",\"params\":{\"tool\":\"lark_task_api\",\"callId\":\"call-90b\",\"threadId\":\"thread-90b\",\"turnId\":\"turn-90b\",\"arguments\":{\"method\":\"GET\",\"path\":\"/open-apis/task/v2/tasks/task-90b\"}}}'
            ;;
          5)
            printf '%s\\n' '{\"method\":\"turn/completed\"}'
            exit 0
      ...
      assert_received {:tool_called, "lark_task_api", %{"method" => "GET", "path" => "/open-apis/task/v2/tasks/task-90b"}}

      assert_received {:app_server_message, %{event: :tool_call_failed, payload: %{"params" => %{"tool" => "lark_task_api"}}}}
    after
      File.rm_rf(test_root)
    end
  end
